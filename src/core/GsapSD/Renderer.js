export default class Renderer {
  ...
  resize() {
    ...
    const needsResize =
      this.gl.canvas.width * this.pixelRatio !== displayWidth ||
      this.gl.canvas.height * this.pixelRatio !== displayHeight;
      ...
      if (bindBuffers || object.attributes != lastBuffer) {
        for (const attribute in object.attributes) {
          ...
          this.gl.vertexAttribPointer(
            object.attributes[attribute].location,
            size,
            type,
            normalize,
            stride,
      ...
      for (const uniform in object.uniforms) {
        if (uniform === 'uViewProjectionMatrix') {
          this.gl.uniformMatrix4fv(
            object.uniforms[uniform].location,
            false,
          ...
          switch (object.uniforms[uniform].type) {
            ...
            case '2f':
              this.gl.uniform2f(
                object.uniforms[uniform].location,
                object.uniforms[uniform].value[0],
                object.uniforms[uniform].value[1]
              );
              break;
            case '3f':
              this.gl.uniform3f(
                ...
                object.uniforms[uniform].value[0],
                object.uniforms[uniform].value[1],
                object.uniforms[uniform].value[2]
              );
              break;
            case '4f':
              this.gl.uniform4f(
                ...
                object.uniforms[uniform].value[1],
                object.uniforms[uniform].value[2],
                object.uniforms[uniform].value[3]
              );
              break;
            case 'mat3':
            ...
            case 'tex':
              this.gl.uniform1i(
                object.uniforms[uniform].location,
                object.uniforms[uniform].value.id
              );
